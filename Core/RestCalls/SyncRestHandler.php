...
class SyncRestHandler extends RestHandler
{
    ...
    public function GetResponseSyncRest($requestParameters, $requestBody, $oauthRequestUri)
    {
        ...
        try {
            ...
            if ($this->context->IppConfiguration->RetryPolicy == null) {
                ...
                list($responseCode, $response) = $this->CallRestService($requestParameters, $requestBody,
                    $oauthRequestUri);
            } else {
                // Not yet implemented
            ...
            if ($idsException != null) {
                $this->context->IppConfiguration->Logger->CustomLogger->Log(TraceLevel::Error,
                    $idsException->getMessage());
                throw $idsException;
            }
        }
    ...
    private function GetOAuthResponseHeaders($oauth)
    {
        ...
        try {
            ...
            if ('QBO' == $this->context->serviceType || 'QBD' == $this->context->serviceType) {
                ...
                if ($this->ResponseCompressor && $response_headers &&
    ...
    public function GetResponse($requestParameters, $requestBody, $oauthRequestUri)
    {
        ...
        $oauth = new OAuth($this->context->requestValidator->ConsumerKey,
            $this->context->requestValidator->ConsumerSecret);
        $oauth->setToken($this->context->requestValidator->AccessToken,
    ...
    {
        $this->context->IppConfiguration->Logger->CustomLogger->Log(TraceLevel::Info,
            "Getting the response from service.");

        // Call the service and get response.
        list($httpWebResponseCode, $httpWebResponseBody) = $this->GetResponse($requestParameters, $requestBody,
            $oauthRequestUri);
